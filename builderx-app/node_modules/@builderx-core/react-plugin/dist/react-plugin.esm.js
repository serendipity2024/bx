import { get, escape, includes, replace, forEach, round, isEmpty, isArray, findIndex, keys, isNil, entries } from 'lodash-es';
import { basename, extname } from 'path';
import pascalCase$8 from 'pascal-case';
...
var jsonHasButtonLayer = function jsonHasButtonLayer(layer, context) {
  layer.children.forEach(function (child) {
    if (get(child, 'import.layerName', 0) && child["import"].layerName === 'TouchableOpacity') {
...
var hasPositionProperty = function hasPositionProperty(layer) {
  var position = get(layer, 'props.style.position', undefined);
...
var addFlexValuesForScrollViewChildren = function addFlexValuesForScrollViewChildren(extraParams, json) {
  var parent = get(extraParams, 'parent', undefined);

  if (parent && parent.type && parent.type.toLowerCase() === 'scrollview') {
    var parentStyles = get(parent, 'props.style', undefined);
...
var LayerCodeGen = /*#__PURE__*/function () {
  function LayerCodeGen() {
    ...
    this.generateProps = function (json, extraParams) {
      ...
      if (json.props) {
        forEach(json.props, function (propValue, propName) {
          ...
          } else if (get(json, 'import.layerName').indexOf('GoogleMapReact') !== -1 && propName === 'initialRegion') {
          ...
          } else if (get(json, 'import.layerName') === 'ReactList' && propName === 'renderItem') {
            propName = 'itemRenderer';
            propsCode += propName + "={({ item, separators }) => " + _this.context.baseCodeGen.generate(propValue, extraParams) + "}";
          } else if (get(json, 'import.layerName') === 'ReactList' && (propName === 'ListHeaderComponent' || propName === 'ListFooterComponent' || propName === 'ItemSeparatorComponent')) {
    ...
    this.generateStyle = function (json, extraParams, stylePropName) {
      ...
      var style = get(json, "props." + stylePropName, {});
    ...
    this.generateStyleComponentCode = function (literal, json, extraParams) {
      ...
      forEach(literal, function (element, key) {
  ...
  _proto.generateStyleCode = function generateStyleCode(obj, extraParams, category, json) {
    ...
    if (flexStyles && category !== 'root') {
      var flexDirection = get(extraParams, 'parent.props.style.flexDirection', undefined);
...
var DivCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generate = function generate(json, extraParams) {
    ...
    var tag = json.type === 'root' && isEmpty(restStyles) ? '' : pascalCase$8(json.styleName);
...
function updateWrapperStyleName(json, extraParams) {
  if (json.type === 'row' || json.type === 'column' || json.type === 'stack') {
    var firstChildStyleName = get(json, 'children.0.styleName', getRandomString(6));
    ...
    if (previousSiblingStyleName && previousSiblingStyleName !== 'flexFiller') {
      ...
      json.styleName = get(extraParams, "parent.children." + (siblingsLength - 1) + ".styleName", getRandomString(6)) + 'Filler';
...
var FileCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generate = function generate(json, extraParams) {
    ...
    var containerHasDefaultStyles = isEmpty(restStyles) && (!display || display === 'flex') && (!flexDirection || flexDirection === 'column');
...
var ImageCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generate = function generate(json, extraParams) {
    ...
    if (json.props.mask) {
      ...
      return "<svg " + this.generateProps(svgJson, extraParams) + " style={" + this.generateJSLiteralCode(json.props.style, extraParams) + "}>\n      " + this.generateDefs(json, maskId) + "\n      <image href={require(\"" + this.generateSourcePropCode(basename(json.props.source.uri), 'svgImage') + "\")} x={" + json.props.svgDims.x + "} y={" + json.props.svgDims.y + "} width={" + json.props.svgDims.width + "} height={" + json.props.svgDims.height + "} mask={\"url(#" + maskId + ")\"}>\n      " + generateChildren(json, this.context.baseCodeGen.generate, {
        parent: json
      }) + "\n      </image>\n      </svg>";
    }
    ...
    if (json.children.length !== 0) {
      ...
      if (!json.props.style.gradient) {
        var assetsRelativePath = this.config.isPreviewFile ? './assets/images/' : '../assets/images/';
        var pathName = assetsRelativePath + basename(json.props.source.uri);
  ...
  function BaseCodeGen() {
    ...
    _this.generate = function (json, extraParams) {
      ...
      if (json.props) {
        ...
        if (json.props.style['position'] === 'absolute' && get(extraParams, 'parent.type') !== 'root' && get(extraParams, 'parent.props.style') && !get(extraParams, 'parent.props.style.position')) {
          extraParams.parent.props.style.position = 'relative';
        }

        if (get(json, 'props.style.position') !== 'absolute' && get(extraParams, 'parent.props.style')) {
...
var ReactEngine = /*#__PURE__*/function () {
  ...
  _proto.getAppFiles = function getAppFiles(files) {
    var router = '';
    var imports = '';
    forEach(files.artboards, function (artboard, index) {
      if (index === 0) {
    ...
    var styleCSSCode = "";
    forEach(files.assets, function (asset) {
      var fontName = basename(asset, extname(asset));
  ...
  _proto.getIndexWithFont = function getIndexWithFont(assets) {
    ...
    forEach(assets, function (asset) {
      var fontName = basename(asset, extname(asset));
      var type = extname(asset).substr(1);
...
export { ReactCodeGen, ReactEngine };
//# sourceMappingURL=react-plugin.esm.js.map
