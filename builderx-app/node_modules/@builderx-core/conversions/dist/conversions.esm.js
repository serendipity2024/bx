...
function getPointsArrayFromPath(path, strokeWidthOffset, isClosed) {
  ...
  for (var _i = 1; _i < pathArray2.length; _i++) {
    ...
    for (var _j = 0; _j < pathArray2[_i].length; _j++) {
      if (!isEmpty(pathArray2[_i]) && pathArray2[_i][_j] && !isEmpty(pathArray2[_i][_j]) && pathArray2[_i][_j][0]) {
...
function getPropsOnCondition(pathSubArray, j, strokeWidthOffset) {
  ...
  var hasCurveFrom = !isEmpty(pathSubArray) && pathSubArray[nextIndex] && !isEmpty(pathSubArray[nextIndex]) && pathSubArray[nextIndex][0] && pathSubArray[nextIndex][0].charAt(0) === "C"; // DOC: checking whether next point has curve and setting curveFrom point acc. to that only

  if (!isEmpty(pathSubArray) && pathSubArray[j] && !isEmpty(pathSubArray[j]) && pathSubArray[j][0] && !isEmpty(pathSubArray[j][0])) {
    if (pathSubArray[j][0].charAt(0) === "C") {
      tempPoint.hasCurveFrom = hasCurveFrom;
      tempPoint.curveFromX = hasCurveFrom && pathSubArray[nextIndex] && !isEmpty(pathSubArray[nextIndex]) ? pathSubArray[nextIndex][0].substring(1) - strokeWidthOffset : pathSubArray[j][0].substring(1) - strokeWidthOffset;
      tempPoint.curveFromY = hasCurveFrom && pathSubArray[nextIndex] && !isEmpty(pathSubArray[nextIndex]) ? pathSubArray[nextIndex][1] - strokeWidthOffset : pathSubArray[j][1] - strokeWidthOffset;
      ...
      tempPoint.curveFromX = pathSubArray[nextIndex] && !isEmpty(pathSubArray[nextIndex]) ? pathSubArray[nextIndex][0].substring(1) - strokeWidthOffset : 0;
      tempPoint.curveFromY = pathSubArray[nextIndex] && !isEmpty(pathSubArray[nextIndex]) ? pathSubArray[nextIndex][1] - strokeWidthOffset : 0;
...
export { applyBooleanOpsOnPaths, convertPaperPointToBxObj, convertPaperToBx, convertPointsArray, generateSVGPath, getPaperPathFromPoints, getPointsArrayFromPath, getRadiusAdjustedPoints, getSegmentFromPoints };
