import { get, escape, includes, replace, forEach, round, isArray, findIndex, keys, isNil, entries } from 'lodash-es';
...
var getIconCount = function getIconCount(context, layer) {
  ...
  layer.children.forEach(function (child) {
    ...
    if (get(child, 'import.layerName', 0) && child["import"].layerName === 'Icon') {
...
var START_STORYBOOK = process.env.NODE_ENV === 'production' ? false : false; // Set this true to use testpad
...
var LayerCodeGen = /*#__PURE__*/function () {
  function LayerCodeGen() {
    ...
    this.generateProps = function (json, extraParams) {
      ...
      if (json.props) {
        ...
        forEach(json.props, function (propValue, propName) {
          if (propName === 'onPress') {
          ...
          } else if (get(json, 'import.layerName') === 'FlatList' && propName === 'renderItem') {
            propsCode += propName + "={({ item, separators }) => " + _this.context.baseCodeGen.generate(propValue, extraParams) + "}";
          } else if (get(json, 'import.layerName') === 'FlatList' && (propName === 'ListHeaderComponent' || propName === 'ListFooterComponent' || propName === 'ItemSeparatorComponent')) {
            propsCode += propName + "={() => " + _this.context.baseCodeGen.generate(propValue, extraParams) + "}";
          } else if (get(json, 'import.layerName') === 'Switch' && (propName === 'onTintColor' || propName === 'tintColor')) {
            var onOffColorCode = '';

            if (get(json, 'props.onTintColor')) {
              onOffColorCode += "true: " + _this.generateJSLiteralCode(get(json, 'props.onTintColor'), extraParams) + ",";
            }

            if (get(json, 'props.tintColor')) {
              onOffColorCode += "false: " + _this.generateJSLiteralCode(get(json, 'props.tintColor'), extraParams);
            }
          ...
          } else if (get(json, 'import.layerName') === 'Switch' && propName === 'thumbTintColor') {
            if (propValue) {
              if (typeof propValue === 'string') {
          ...
          } else if (get(json, 'import.layerName') === 'MapView' && propName === 'provider') {
            propsCode += propName + "={" + _this.generateJSLiteralCode(propValue, extraParams, true) + "}";
          } else if (propName === 'source') {
            propsCode += _this.generateSourcePropCode(basename(propValue.uri));
          } else if (get(json, 'import.layerName') === 'MapView' && propName === 'customMapStyle') {
            propValue = MapStyles[propValue];
            propsCode += propName + "={" + _this.generateJSLiteralCode(propValue, extraParams) + "}";
          } else if (get(json, 'import.layerName') === 'Icon' && propName === 'type') ; else if (!isTabViewSpecificStyleProp(propName) && propName !== 'style' && propName !== 'contentContainerStyle' && propName !== 'imageStyle') {
            if (typeof propValue === 'string') {
              propsCode += propName + "=" + _this.generateJSLiteralCode(propValue, extraParams);
            } else {
    ...
    this.generateStyle = function (json, extraParams, stylePropName) {
      ...
      var style = get(json, "props." + stylePropName, {}); // if (stylePropName === "imageStyle" && Object.keys(style).length === 0) {
      //   return "";
      // }
      // if (
      //   json.type === "root" &&
      ...
      forEach(style, function (styleValue, styleProperty) {
      ...
      if (Object.keys(conditionalStyles).length !== 0) {
        ...
        forEach(conditionalStyles, function (code, styleProperty) {
  ...
  _proto.generateSymbolOverrideProps = function generateSymbolOverrideProps(scopeVariables, extraParams) {
    ...
    var code = '';
    forEach(scopeVariables, function (value, key) {
...
var ViewCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generate = function generate(json, extraParams) {
    ...
    if (json.props.style.gradient) {
      ...
      var width = get(json, 'props.style.width');
      var height = get(json, 'props.style.height');
...
var FileCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generateImports = function generateImports() {
    var importsCode = '';
    forEach(this.context.imports, function (importObj, packageName) {
      ...
      forEach(importObj.named, function (layerName, index) {
        if (index === 0) {
          namedImportsCode += '{ ';
        }
  ...
  _proto.generate = function generate(json, extraParams) {
    ...
    if (json.props.mask) {
      ...
      return "<Svg " + this.generateProps(svgJson, _extends({}, extraParams, {
        ...
        parent: json
      })) + ">\n      " + this.generateDefs(json, maskId) + "\n      <SvgImage href={require(\"" + this.generateSourcePropCode(basename(json.props.source.uri), 'svgImage') + "\")} x={" + json.props.svgDims.x + "} y={" + json.props.svgDims.y + "} width={" + json.props.svgDims.width + "} height={" + json.props.svgDims.height + "} mask={\"url(#" + maskId + ")\"}>\n      " + this.generateChildren(json, _extends({}, extraParams, {
        parent: json
      })) + "\n      </SvgImage>\n      </Svg>";
    }
...
function getScrollViewContentContainerStyle(layer) {
  ...
  forEach(layer.props.style, function (value, key) {
...
function getTotalHeightForScrollView(layer) {
  ...
  forEach(layer.children, function (compiledChild) {
...
var TextInputCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generate = function generate(json, extraParams) {
    ...
    if (includes(json.props.placeholder, '\n')) {
      json.props.placeholder = replace(json.props.placeholder, new RegExp('\n', 'g'), ' ');
      json.props.placeholder = replace(json.props.placeholder, new RegExp('\t', 'g'), ' ');
    }
...
var ConditionalExpressionCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generate = function generate(json, extraParams) {
    var expression = get(json, 'test.expression', undefined);
...
var PathCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  _proto.generateDefs = function generateDefs(json) {
    ...
    if (gradient && gradient.gradientType === 'LinearGradient') {
      ...
      var stopsCode = '';
      forEach(gradient.stops, function (stop) {
...
function updateWrapperStyleName(json, extraParams) {
  if (json.type === 'row' || json.type === 'column' || json.type === 'stack') {
    var firstChildStyleName = get(json, 'children.0.styleName', randomString(6));
    json.styleName = firstChildStyleName + json.type.charAt(0).toUpperCase() + json.type.slice(1);
  } else if (json.type === 'flexFiller') {
    var siblingsLength = get(extraParams, 'parent.children.length', 1);
    var previousSiblingStyleName = get(extraParams, 'parent.children.0.styleName');

    if (previousSiblingStyleName && previousSiblingStyleName !== 'flexFiller') {
      json.styleName = previousSiblingStyleName + 'Filler';
    } else {
      json.styleName = get(extraParams, "parent.children." + (siblingsLength - 1) + ".styleName", randomString(6)) + 'Filler';
...
function getRotation(transform) {
  ...
  if (transform) {
    var transformValue = isArray(transform) ? transform : transform['value'];
...
var BaseCodeGen = /*#__PURE__*/function (_LayerCodeGen) {
  ...
  function BaseCodeGen() {
    ...
    _this.componentCodeGenMap = new Map(entries({
      view: ViewCodeGen,
      root: RootCodeGen,
      file: FileCodeGen,
      text: TextCodeGen,
    ...
    _this.generate = function (json, extraParams) {
      ...
      var display = get(json, 'props.style.display', undefined);

      if (display === 'flex') {
        delete json.props.style.display;
      }

      var flexDirection = get(json, 'props.style.flexDirection', undefined);

      if (flexDirection === 'column') {
        delete json.props.style.flexDirection;
      }

      var opacity = get(json, 'props.style.opacity', undefined);

      if (opacity === 1) {
        delete json.props.style.opacity;
      }

      var borderRadius = get(json, 'props.style.borderRadius', undefined);

      if (borderRadius === 0) {
        delete json.props.style.borderRadius;
      }

      var transform = get(json, 'props.style.transform', undefined);
      ...
      var sourceLocal = get(json, 'props.source_local', undefined);

      if (sourceLocal) {
        delete json.props.source_local;
      }

      if (get(json, 'template.centerVertical') || get(json, 'template.centerHorizontal')) {
...
var ReactNativeEngine = /*#__PURE__*/function () {
  ...
  _proto.getAppFiles = function getAppFiles(files) {
    ...
    var navigator = "";
    var imports = "";
    forEach(files.artboards, function (artboard) {
      navigator += artboard + ":" + artboard + ",";
      imports += "import " + artboard + " from \"./src/screens/" + artboard + "\";";
    });
    stackNavigator += navigator;
    var fonts = '';
    forEach(files.assets, function (font) {
      fonts += "\"" + basename(font, extname(font)) + "\": require(\"." + font + "\"),";
    });
    forEach(files.missingAssets, function (missingAsset) {
      fonts += "\"" + basename(missingAsset.file.path, extname(missingAsset.file.path)) + "\": require(\"." + missingAsset.fallbackPath + "\"),";
    });
  ...
  _proto.getIndexWithFont = function getIndexWithFont(assets) {
    var fonts = '';
    forEach(assets, function (font) {
      fonts += "\"" + basename(font, extname(font)) + "\": require(\"." + font.replace('src/assets', 'assets') + "\"),";
    });
...
ReactNativeEngine.engineName = 'react-native';
...
export { ReactNativeCodeGen, ReactNativeEngine };
//# sourceMappingURL=react-native-plugin.esm.js.map
