import { pickBy, isNil, get } from "lodash";
...
export default function getStyle(
  ...
  try {
    let width =
      convertedFromSvg && layer.layers[0].frame
        ? layer.layers[0].frame.width
        : layer.frame.width + borderThickness * 2;
    let height =
      convertedFromSvg && layer.layers[0].frame
        ? layer.layers[0].frame.height
    ...
    let points = get(layer, "layers[0].points");
    if (!points) {
      points = get(layer, "layers[0].path.points");
    }
    if (isAvatar) {
      ...
      layer.layers &&
      layer.layers.length &&
      layer.layers[0].fixedRadius &&
      layer.layers[0].fixedRadius !== 0 &&
      points &&
      points.length === 4 &&
      points[0].cornerRadius === points[1].cornerRadius &&
      points[1].cornerRadius === points[2].cornerRadius &&
      points[2].cornerRadius === points[3].cornerRadius
    ) {
      style.borderRadius = layer.layers[0].fixedRadius;
    } else if (
      layer.layers &&
      layer.layers.length &&
      points &&
      ...
      if (points[0].cornerRadius !== 0) {
        style.borderTopLeftRadius = points[0].cornerRadius;
      }
      if (points[1].cornerRadius !== 0) {
        style.borderTopRightRadius = points[1].cornerRadius;
      }
      if (points[2].cornerRadius !== 0) {
        style.borderBottomRightRadius = points[2].cornerRadius;
      }
      if (points[3].cornerRadius !== 0) {
        style.borderBottomLeftRadius = points[3].cornerRadius;
      }
    }
